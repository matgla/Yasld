...
#include "msos/dynamic_linker/logging.hpp"
...
bool DynamicLinker::allocate_lot(LoadedModule& loaded_module)
{
    ...
    log_debug("Allocating lot with size: %d\n", header.number_of_external_relocations() + header.number_of_local_relocations());
    auto& lot = module.get_lot();
    lot.reset(new std::size_t[header.number_of_external_relocations() + header.number_of_local_relocations()]);

    log_info("Allocated lot at: %p\n", static_cast<void*>(lot.get()));
    return lot != nullptr;
}
